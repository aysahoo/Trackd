...
export default function MovieDetailsModal({ movie, isOpen, onClose }: MovieDetailsModalProps) {
  ...
  const ModalContent = ({ showDragHandle = false, showCloseButton = true }: { showDragHandle?: boolean; showCloseButton?: boolean }) => (
      ...
      <Drawer.Root open={isOpen} onOpenChange={(open) => !open && onClose()}>
        <Drawer.Portal>
          ...
          <Drawer.Content className="fixed inset-x-0 bottom-0 z-[101] flex flex-col items-center outline-none">
            ...
            <Drawer.Description className="sr-only">{movie.year} • {movie.type}</Drawer.Description>
  ...
  return (
    <>
      ...
      >
        <div
          className={`w-full max-w-sm transition-transform duration-300 ease-out ${isOpen ? 'scale-100' : 'scale-95'}`}
